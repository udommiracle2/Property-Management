...
const router = Router();
const crud = createCrudHandlers(Message, { required: ["id"] });
...
router.post("/:id/reply", async (req, res) => {
  try {
    ...
    const reply = {
      text,
      from: from || req.user.name || req.user.email,
      at: todayISO(),
    };
    msg.thread = msg.thread || [];
    msg.thread.push(reply);
    msg.preview = text;
    msg.unread = req.user.role === "tenant"; // mark unread for admin if tenant replies
    await msg.save();
